...
export default {
  async fetch(req, env) {
    ...
    return new Response(
      `<!DOCTYPE html><html lang="zh-cn"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><style>textarea{font-family:inherit;font-size:16px;background-color:inherit;}.btn{background-color:#AAAAAAB2;border:none;border-radius:8px;transition:transform 0.3s, background-color 0.5s;}.btn:hover{transform:translateY(-1.5px) scaleX(1.03);background-color:#A1A1A1F2;}#in{border-radius:8px;background-color:#E6E6E67F;transition:transform 0.3s, background-color 0.3s;}#in:hover{transform:translateY(-1.5px) scaleX(1.003);background-color:#DCDCDCB2;}</style></head><span style="font-size:26px;color: #888888">评论</span><form method="post"><div style="display:flex;"><textarea name="txt" rows="1" oninput="resizeTextarea(this)"style="flex:1; resize:none" id="in"></textarea><input type="submit"value="提交"style="align-self:stretch;height:auto;margin-left:4px;"class="btn"></div></form><textarea readonly style="width:100%;resize:none;border:none; outline:none;color: #888888" id='0'>${txt}</textarea><script>function resizeTextarea(textarea){textarea.style.height = 'auto';textarea.style.height = textarea.style.height = (textarea.scrollHeight) + 'px';}resizeTextarea(textarea = document.getElementById('0'));resizeTextarea(textarea = document.getElementById("in"))</script>`,
      {
        headers: { "Content-Type": "text/html" },
      }
    );
